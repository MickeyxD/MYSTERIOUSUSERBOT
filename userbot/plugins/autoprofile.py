...
DEFAULTUSERBIO = str(BIO_MSG) if BIO_MSG else "Legendary MYSTERIOUSBOT"
DEL_TIME_OUT = 60
...
async def _(event):
    if event.fwd_from:
        return
    MYSTERIOUS = await edit_or_reply(event, "`Starting AutoName Please Wait`")
    if event.fwd_from:
        return
    ...
    await MYSTERIOUS.edit(f"Auto Name has been started my Master")
